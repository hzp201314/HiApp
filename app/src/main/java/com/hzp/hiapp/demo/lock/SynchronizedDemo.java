...
/**
 * 卖票并发场景--不设置并发安全措施，会出现多人买同一张票
 *
 * 如果synchronized 加在方法上。未获取到对象锁的其他线程都不可以访问该方法，只能排队。
 * 如果synchronized 加在代码块上。未获取到对象锁的其他线程可以执行同步代码块之外的代码
 * 如果synchronized 加在static方法上。就相当于给Class对象加锁，由于JVM中只会存在一份class对象。
 *      所以此时无论是不是同一个java对象，去访问同步代码块，都只能排队。
 */
